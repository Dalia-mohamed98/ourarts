...
class Dokan_Template_Products {
    ...
    public function render_product_listing_template( $action ) {
        $bulk_statuses = apply_filters( 'dokan_bulk_product_statuses', array(
            '-1'     => __( 'عمل للكل', 'dokan-lite' ),
            'delete' => __( 'لحذف بشكل نهائي', 'dokan-lite' ),
        ) );
    ...
    function handle_product_add() {
        ...
        $errors             = array();
        self::$product_cat  = -1;
        self::$post_content = __( 'تفاصيل المنتج الخاص بك ...', 'dokan-lite' );
        ...
        if ( isset( $postdata['add_product'] ) ) {
            ...
            if ( empty( $post_title ) ) {
                $errors[] = __( 'الرجاء إدخال عنوان المنتج', 'dokan-lite' );
            }
            ...
            if ( dokan_get_option( 'product_category_style', 'dokan_selling', 'single' ) == 'single' ) {
                ...
                if ( $product_cat < 0 ) {
                    $errors[] = __( 'الرجاء تحديد الفئة', 'dokan-lite' );
                }

            } else {
                if ( ! isset( $postdata['product_cat'] ) && empty( $postdata['product_cat'] ) ) {
                    $errors[] = __( 'يرجى اختيار فئة واحدة على الأقل', 'dokan-lite' );
                }
            }
    ...
    public function handle_product_update() {
        ...
        if ( ! $post_id ) {
            $errors[] = __( 'لم يتم العثور على المنتج!', 'dokan-lite' );
        }

        if ( ! dokan_is_product_author( $post_id ) ) {
            $errors[] = __( 'أقسم أن هذا ليس منتجك!', 'dokan-lite' );
        }
