...
class Dokan_Template_Withdraw extends Dokan_Withdraw {
    ...
    public function withdraw_header_render() {
        ...
        if ( ! current_user_can( 'dokan_manage_withdraw' ) ) {
            dokan_get_template_part('global/dokan-error', '', array(
                'deleted' => false,
                'message' => __( 'ليس لديك إذن لإدارة عمليات السحب', 'dokan-lite' ),
            ) );
            return;
        }
    }
    ...
    public function show_seller_balance() {
        ...
        $message = sprintf( __( 'الرصيد الحالي: %s ', 'dokan-lite' ), $balance );

        if ( $withdraw_limit != -1 ) {
            $message .= sprintf( __( '<br>الحد الأدنى لسحب المبلغ: %s ', 'dokan-lite' ), wc_price( $withdraw_limit ) );
        }

        if ( $threshold != -1 ) {
            $message .= sprintf( __( '<br>سحب العتبة: %d يوم ', 'dokan-lite' ), $threshold );
        }
    ...
    function validate() {
        ...
        if ( ! current_user_can( 'dokan_manage_withdraw' ) ) {
            wp_die( esc_attr__( 'ليس لديك إذن للقيام بهذا الإجراء', 'dokan-lite' ) );
        }
        ...
        if ( empty( $withdraw_amount ) ) {
            $error->add( 'dokan_empty_withdrad', __( 'سحب المبلغ المطلوب', 'dokan-lite' ) );
        } elseif ( $withdraw_amount > $balance ) {
            $error->add( 'enough_balance', __( 'ليس لديك رصيد كافي لهذا الطلب', 'dokan-lite' ) );
        } elseif ( $withdraw_amount < $limit ) {
            $error->add( 'dokan_withdraw_amount', sprintf( __( 'يجب أن يكون مبلغ السحب أكبر من %d', 'dokan-lite' ), $this->get_withdraw_limit() ) );
        }

        if ( empty( sanitize_text_field( $post_data['withdraw_method'] ) ) ) {
            $error->add( 'dokan_withdraw_method', __( 'طريقة السحب مطلوبة', 'dokan-lite' ) );
        }
    ...
    function show_alert_messages() {
        ...
        switch ( $type ) {
            case 'request_cancelled':
                $message = __( 'تم إلغاء طلبك بنجاح!', 'dokan-lite' );
                break;

            case 'request_success':
                $message = __( 'تم استلام طلبك بنجاح ويتم مراجعته!', 'dokan-lite' );
                break;

            case 'request_error':
                $message = __( 'خطأ غير معروف!', 'dokan-lite' );
    ...
    function withdraw_form( $validate = '' ) {
        ...
        if ( $balance < 0 ) {
            dokan_get_template_part( 'global/dokan-error', '', array(
                'deleted' => false,
                'message' => sprintf( __( 'لقد سحبت بالفعل %s. سيتم خصم هذا المبلغ من رصيدك.', 'dokan-lite' ), wc_price( $balance ) ),
            ) );
        }
        ...
        if ( $this->has_pending_request( $current_user->ID ) ) {
            ...
            if ( ! $req_success ) {
                $pending_warning = sprintf( '<p>%s</p><p>%s</p>', __( 'لديك بالفعل طلب (طلبات) السحب المعلقة.', 'dokan-lite' ), __( 'يرجى تقديم طلبك بعد الموافقة أو إلغاء طلبك السابق.', 'dokan-lite' ) );
            ...
            dokan_get_template_part( 'global/dokan-error', '', array(
                'deleted' => false,
                'message' => __( 'ليس لديك رصيد كافي لطلب السحب!', 'dokan-lite' ),
            ) );
    ...
    function user_approved_withdraws( $user_id ) {
        ...
        if ( $requests ) {
            ...
            dokan_get_template_part( 'global/dokan-warning', '', array(
                'deleted' => false,
                'message' => __( 'آسف ، لم يتم العثور على المعاملات!', 'dokan-lite' ),
            ) );
