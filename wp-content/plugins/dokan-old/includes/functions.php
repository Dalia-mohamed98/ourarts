...
function dokan_get_store_tabs( $store_id ) {
    $tabs = array(
        ...
        'terms_and_conditions' => array(
            'title' => __( 'الشروط و الأحكام', 'dokan-lite' ),
...
function dokan_get_seller_bank_details( $seller_id ) {
    ...
    if ( isset( $payment['ac_name'] ) ) {
        $details[] = sprintf( __( 'اسم الحساب: %s', 'dokan-lite' ), $payment['ac_name'] );
    }

    if ( isset( $payment['ac_number'] ) ) {
        $details[] = sprintf( __( 'رقم الحساب: %s', 'dokan-lite' ), $payment['ac_number'] );
    }

    if ( isset( $payment['bank_name'] ) ) {
        $details[] = sprintf( __( 'اسم البنك: %s', 'dokan-lite' ), $payment['bank_name'] );
    }

    if ( isset( $payment['bank_addr'] ) ) {
        $details[] = sprintf( __( 'العنوان: %s', 'dokan-lite' ), $payment['bank_addr'] );
    }

    if ( isset( $payment['routing_number'] ) ) {
        $details[] = sprintf( __( 'رقم التعقب: %s', 'dokan-lite' ), $payment['routing_number'] );
    }
...
function dokan_country_dropdown( $options, $selected = '', $everywhere = false ) {
    printf( '<option value="">%s</option>', esc_html__( '- اختر موقع -', 'dokan-lite' ) );

    if ( $everywhere ) {
        echo '<optgroup label="--------------------------">';
        printf( '<option value="everywhere"%s>%s</option>', selected( $selected, 'في كل مكان', true ), esc_html__( 'فى اى مكان اخر', 'dokan-lite' ) );
        echo '</optgroup>';
    }
...
function dokan_state_dropdown( $options, $selected = '', $everywhere = false ) {
    printf( '<option value="">%s</option>', esc_html__( '- اختر الدولة -', 'dokan-lite' ) );

    if ( $everywhere ) {
        echo '<optgroup label="--------------------------">';
        printf( '<option value="everywhere" %s>%s</option>', selected( $selected, 'فى اى مكان', true ), esc_html__( 'فى اى مكان اخر', 'dokan-lite' ) );
        echo '</optgroup>';
    }
...
function dokan_get_translated_days( $day ) {
    switch ( $day ) {
        case 'saturday':
            return __( 'السبت', 'dokan-lite' );

        case 'sunday':
            return __( 'الأحد', 'dokan-lite' );

        case 'monday':
            return __( 'الإثنين', 'dokan-lite' );

        case 'tuesday':
            return __( 'الثلاثاء', 'dokan-lite' );

        case 'wednesday':
            return __( 'الأربعاء', 'dokan-lite' );

        case 'thursday':
            return __( 'الخميس', 'dokan-lite' );

        case 'friday':
            return __( 'الجمعة', 'dokan-lite' );

        case 'close':
            return apply_filters( 'dokan_store_close_day_label', __( 'يوم مغلق', 'dokan-lite' ) );
