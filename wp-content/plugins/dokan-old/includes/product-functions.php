...
function dokan_save_product( $args ) {
    ...
    if ( empty( $data['post_title'] ) ) {
        return new WP_Error( 'no-title', __( 'الرجاء إدخال عنوان المنتج', 'dokan-lite' ) );
    }

    if ( dokan_get_option( 'product_category_style', 'dokan_selling', 'single' ) == 'single' ) {
        $product_cat    = absint( $data['product_cat'] );
        if ( $product_cat < 0 ) {
            return new WP_Error( 'no-category', __( 'الرجاء تحديد الفئة', 'dokan-lite' ) );
        }
    } else {
        if ( ! isset( $data['product_cat'] ) && empty( $data['product_cat'] ) ) {
            return new WP_Error( 'no-category', __( 'يرجى اختيار فئة واحدة على الأقل', 'dokan-lite' ) );
        }
    }
    ...
    if ( ! empty( $data['ID'] ) ) {
        ...
        if ( ! dokan_is_product_author( $post_arr['ID'] ) ) {
            return new WP_Error( 'not-own', __( 'أقسم أن هذا ليس منتجك!', 'dokan-lite' ) );
        }
    ...
    <div id="dokan-variable-product-options" class="">
        ...
        <?php else : ?>
            ...
            <div class="dokan-variation-top-toolbar">
                <div class="dokan-variation-label content-half-part">
                    <div class="toolbar toolbar-top">
                        <select id="field_to_edit" class="variation-actions dokan-form-control dokan-w10">
                            <option data-global="true" value="add_variation"><?php esc_html_e( 'إضافة التصنيف', 'dokan-lite' ); ?></option>
                            <option data-global="true" value="link_all_variations"><?php esc_html_e( 'إنشاء أشكال مختلفة من جميع السمات', 'dokan-lite' ); ?></option>
                            <option value="delete_all"><?php esc_html_e( 'حذف جميع الاختلافات', 'dokan-lite' ); ?></option>
                            <optgroup label="<?php esc_attr_e( 'الحالة', 'dokan-lite' ); ?>">
                            ...
                            <optgroup label="<?php esc_attr_e( 'السعر', 'dokan-lite' ); ?>">
                                <option value="variable_regular_price"><?php esc_html_e( 'تحديد الأسعار العادية', 'dokan-lite' ); ?></option>
                                <option value="variable_regular_price_increase"><?php esc_html_e( 'زيادة الأسعار العادية (المبلغ الثابت أو النسبة المئوية)', 'dokan-lite' ); ?></option>
                                <option value="variable_regular_price_decrease"><?php esc_html_e( 'خفض الأسعار العادية (المبلغ الثابت أو النسبة المئوية)', 'dokan-lite' ); ?></option>
                                <option value="variable_sale_price"><?php esc_html_e( 'تحديد أسعار البيع', 'dokan-lite' ); ?></option>
                                <option value="variable_sale_price_increase"><?php esc_html_e( 'زيادة أسعار البيع (المبلغ الثابت أو النسبة المئوية)', 'dokan-lite' ); ?></option>
                                <option value="variable_sale_price_decrease"><?php esc_html_e( 'خفض أسعار البيع (المبلغ الثابت أو النسبة المئوية)', 'dokan-lite' ); ?></option>
                                <option value="variable_sale_schedule"><?php esc_html_e( 'تحديد مواعيد البيع المجدولة', 'dokan-lite' ); ?></option>
                            </optgroup>
                            <optgroup label="<?php esc_attr_e( 'Inventory', 'dokan-lite' ); ?>">
                                ...
                                <option value="variable_stock"><?php esc_html_e( 'المخزون', 'dokan-lite' ); ?></option>
                            </optgroup>
                            <optgroup label="<?php esc_attr_e( 'الشحن', 'dokan-lite' ); ?>">
                                <option value="variable_length"><?php esc_html_e( 'الطول', 'dokan-lite' ); ?></option>
                                <option value="variable_width"><?php esc_html_e( 'العرض', 'dokan-lite' ); ?></option>
                                <option value="variable_height"><?php esc_html_e( 'الإرتفاع', 'dokan-lite' ); ?></option>
                                <option value="variable_weight"><?php esc_html_e( 'الوزن', 'dokan-lite' ); ?></option>
                            </optgroup>
                            <optgroup label="<?php esc_attr_e( ' المنتجات قابلة للتحميل', 'dokan-lite' ); ?>">
                                <option value="variable_download_limit"><?php esc_html_e( 'حد التنزيل', 'dokan-lite' ); ?></option>
                                <option value="variable_download_expiry"><?php esc_html_e( 'تنزيل انتهاء الصلاحية', 'dokan-lite' ); ?></option>
                            </optgroup>
                            <?php do_action( 'dokan_variable_product_bulk_edit_actions' ); ?>
                        </select>
                        <a class="dokan-btn dokan-btn-default do_variation_action"><?php esc_html_e( 'اذهب', 'dokan-lite' ); ?></a>
                    </div>
                </div>
                ...
                <div class="dokan-variation-default-toolbar content-half-part">
                    ...
                    <div class="dokan-variations-defaults">
                        <span class="dokan-variation-default-label dokan-left float-none"><i class="fa fa-question-circle tips" title="<?php esc_attr_e( 'قيم النموذج الافتراضية: هذه هي السمات التي سيتم اختيارها مسبقًا على الواجهة الأمامية.', 'dokan-lite' ); ?>" aria-hidden="true"></i></span>
                        ...
                        <?php
    						...
    						foreach ( $attributes as $attribute ) {
    							...
    							echo '<select class="dokan-form-control" name="default_attribute_' . esc_attr( sanitize_title( $attribute['name'] ) ) . '" data-current="' . esc_attr( $variation_selected_value ) . '"><option value="">' . esc_html__( 'لا الافتراضي', 'dokan-lite' ) . ' ' . esc_html( wc_attribute_label( $attribute['name'] ) ) . '&hellip;</option>';
            ...
            <div class="dokan-variation-action-toolbar">
                ...
                <button class="dokan-btn dokan-btn-default cancel-variation-changes"><?php esc_html_e( 'إلغاء', 'dokan-lite' ); ?></button>
                ...
                <div class="dokan-variations-pagenav dokan-right">
                    ...
                    <span class="expand-close">
                        (<a href="#" class="expand_all"><?php esc_html_e( 'توسع', 'dokan-lite' ); ?></a> / <a href="#" class="close_all"><?php esc_html_e( 'غلق', 'dokan-lite' ); ?></a>)
                    </span>
                    <span class="pagination-links">
                        <a class="first-page disabled" title="<?php esc_attr_e( 'انتقل إلى الصفحة الأولى', 'dokan-lite' ); ?>" href="#">&laquo;</a>
                        <a class="prev-page disabled" title="<?php esc_attr_e( 'اذهب الى الصفحة السابقة', 'dokan-lite' ); ?>" href="#">&lsaquo;</a>
                        <span class="paging-select">
                            <label for="current-page-selector-1" class="screen-reader-text"><?php esc_html_e( 'اختر الصفحة', 'dokan-lite' ); ?></label>
                            <select class="page-selector" id="current-page-selector-1" title="<?php esc_attr_e( 'الصفحه الحاليه', 'dokan-lite' ); ?>">
