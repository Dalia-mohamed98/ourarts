...
class Dokan_Vendor {
    ...
    */
    ...
    public function get_readable_rating( $display = true ) {
        ...
        if ( ! $rating['count'] ) {
            $html = __( 'لم يتم العثور على تقييمات بعد!', 'dokan-lite' );
        } else {
            $long_text   = _n( '%s التقييم من %d عرض', '%s التقييم من %d عرض', $rating['count'], 'dokan-lite' );
            $text        = sprintf( __( 'تقييمه %s من %d', 'dokan-lite' ), $rating['rating'], number_format( 5 ) );
