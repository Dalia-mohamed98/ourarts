...
function dokan_process_product_meta( $post_id, $data = [] ) {
    ...
    if ( '' == $new_sku ) {
        ...
        if ( ! empty( $new_sku ) ) {
            ...
            if ( ! $unique_sku ) {
                $woocommerce_errors[] = __( 'يجب أن يكون المنتج SKU فريدة من نوعها', 'dokan-lite' );
            } else {
...
function dokan_save_account_details() {
    ...
    if ( $account_email ) {
        if ( ! is_email( $account_email ) ) {
            wc_add_notice( __( 'يرجى تقديم عنوان بريد إلكتروني صالح.', 'dokan-lite' ), 'error' );
        } elseif ( email_exists( $account_email ) && $account_email !== $current_user->user_email ) {
            wc_add_notice( __( 'هذا البريد الالكتروني مسجل سابقا.', 'dokan-lite' ), 'error' );
        }
        $user->user_email = $account_email;
    }

    if ( ! empty( $pass1 ) && ! wp_check_password( $pass_cur, $current_user->user_pass, $current_user->ID ) ) {
        wc_add_notice( __( 'كلمة المرور الحالية غير صحيحة.', 'dokan-lite' ), 'error' );
        $save_pass = false;
    }

    if ( ! empty( $pass_cur ) && empty( $pass1 ) && empty( $pass2 ) ) {
        wc_add_notice( __( 'يرجى ملء جميع حقول كلمة المرور.', 'dokan-lite' ), 'error' );
        $save_pass = false;
    } elseif ( ! empty( $pass1 ) && empty( $pass_cur ) ) {
        wc_add_notice( __( 'الرجاء إدخال كلمة المرور الحالية.', 'dokan-lite' ), 'error' );
        $save_pass = false;
    } elseif ( ! empty( $pass1 ) && empty( $pass2 ) ) {
        wc_add_notice( __( 'من فضلك أعد إدخال كلمة المرور.', 'dokan-lite' ), 'error' );
        $save_pass = false;
    } elseif ( ( ! empty( $pass1 ) || ! empty( $pass2 ) ) && $pass1 !== $pass2 ) {
        wc_add_notice( __( 'كلمات المرور الجديدة غير متطابقة.', 'dokan-lite' ), 'error' );
        $save_pass = false;
    }
    ...
    if ( wc_notice_count( 'error' ) === 0 ) {
        ...
        wc_add_notice( __( 'تم تغيير تفاصيل الحساب بنجاح.', 'dokan-lite' ) );
...
function dokan_set_more_from_seller_tab( $tabs ) {
    if ( check_more_seller_product_tab() ) {
        $tabs['more_seller_product'] = array(
            'title'     => __( 'المزيد من المنتجات', 'dokan-lite' ),
            'priority'  => 99,
...
function dokan_get_more_products_from_seller( $seller_id = 0, $posts_per_page = 6 ) {
    ...
    if ( $products->have_posts() ) {
        ...
        esc_html_e( 'لم يتم العثور على أي منتج!', 'dokan-lite' );
    }
