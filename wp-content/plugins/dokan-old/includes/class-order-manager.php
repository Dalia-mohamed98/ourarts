...
class Dokan_Order_Manager {
    ...
    function on_sub_order_change( $order_id, $old_status, $new_status ) {
        ...
        if ( $all_complete ) {
            ...
            $parent_order->update_status( 'wc-completed', __( 'وضع علامة على الطلب الرئيسي عند الانتهاء من جميع الطلبات الفرعية', 'dokan-lite' ) );
        }
    }
    ...
    function maybe_split_orders( $parent_order_id ) {
        ...
        dokan_log( sprintf( 'طلب جديد #%d انشأ. Init sub order.', $parent_order_id ) );
