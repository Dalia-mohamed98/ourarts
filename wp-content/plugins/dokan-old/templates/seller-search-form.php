<?php if ( ! empty( $search_query ) ): ?>
    <h2><?php printf( __( 'نتائج البحث عن: %s', 'dokan-lite' ), esc_attr( $search_query ) ); ?></h2>
<?php endif; ?>

<form role="search" method="get" class="dokan-seller-search-form" action="">
    <div class="dokan-row dokan-clearfix">
        <div class="dokan-w4">
            <input type="search" id="search" class="search-field dokan-form-control dokan-seller-search" placeholder="<?php esc_attr_e( 'بحث التاجر &hellip;', 'dokan-lite' ); ?>" value="<?php echo esc_attr( $search_query ); ?>" name="dokan_seller_search" title="<?php esc_attr_e( 'بحث البائع &hellip;', 'dokan-lite' ); ?>" />
        </div>
