            ...
            <div class="dokan-widget-area widget-collapse">
                ...
                if ( ! dynamic_sidebar( 'sidebar-store' ) ) {
                    ...
                    if ( class_exists( 'Dokan_Store_Location' ) ) {
                        the_widget( 'Dokan_Store_Category_Menu', array( 'title' => __( 'فئات منتجات المتجر', 'dokan-lite' ) ), $args );

                        if ( dokan_get_option( 'store_map', 'dokan_appearance', 'on' ) == 'on'  && !empty( $map_location ) ) {
                            the_widget( 'Dokan_Store_Location', array( 'title' => __( 'موقع المتجر', 'dokan-lite' ) ), $args );
                        }

                        if ( dokan_get_option( 'store_open_close', 'dokan_appearance', 'on' ) == 'on' ) {
                            the_widget( 'Dokan_Store_Open_Close', array( 'title' => __( 'وقت المتجر', 'dokan-lite' ) ), $args );
                        }

                        if ( dokan_get_option( 'contact_seller', 'dokan_appearance', 'on' ) == 'on' ) {
                            the_widget( 'Dokan_Store_Contact_Form', array( 'title' => __( 'التواصل مع المتجر', 'dokan-lite' ) ), $args );
                        }
                    }
                }
                ?>
    ...
    <div id="dokan-primary" class="dokan-single-store dokan-w12">
        <div id="dokan-content" class="store-page-wrap woocommerce" role="main">
            ...
            <?php } else { ?>
                ...
                <p class="dokan-info"><?php esc_html_e( 'لم يتم العثور على منتجات لهذا التاجر!', 'dokan-lite' ); ?></p>
