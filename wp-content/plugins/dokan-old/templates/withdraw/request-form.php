...
<form class="dokan-form-horizontal withdraw" role="form" method="post">
    ...
    <div class="dokan-form-group">
        <label for="withdraw-amount" class="dokan-w3 dokan-control-label">
            <?php esc_html_e( 'سحب المبلغ', 'dokan-lite' ); ?>
        </label>
    ...
    <div class="dokan-form-group">
        <label for="withdraw-method" class="dokan-w3 dokan-control-label">
            <?php esc_html_e( 'طريقة الدفع', 'dokan-lite' ); ?>
        </label>
    ...
    <div class="dokan-form-group">
        <div class="dokan-w3 ajax_prev" style="margin-left:19%; width: 200px;">
            <?php wp_nonce_field( 'dokan_withdraw', 'dokan_withdraw_nonce' ); ?>
            <input type="submit" class="dokan-btn dokan-btn-theme" value="<?php esc_attr_e( 'تقديم الطلب', 'dokan-lite' ); ?>" name="withdraw_submit">
        </div>
    </div>
</form>
<?php else : ?>
    <div class="dokan-alert dokan-alert-warning">
        <strong><?php echo sprintf( '%s <a href="%s">%s</a>', esc_attr__( 'لا توجد طريقة سحب متاحة. يرجى تحديث طريقة الدفع لسحب الأموال.', 'dokan-lite' ), esc_url( dokan_get_navigation_url( 'settings/payment' ) ), esc_attr__( 'إعداد إعدادات الدفع', 'dokan-lite' ) ) ?></strong>
    </div>
<?php endif; ?>
