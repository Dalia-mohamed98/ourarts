 ...
 */
?>
<h2><?php esc_html_e( 'أرباح', 'affiliates' ); ?></h2>
<?php
	//
...
<div class="dashboard-section dashboard-section-earnings">
	<form id="setfilters" class="filters capsule-container" action="" method="post">
		<div class="capsule half left">
			<label for="from_date" class="from-date-filter"><?php _e( 'منذ', 'affiliates' ); ?></label>
		...
		<div class="capsule half right">
			<label for="thru_date" class="thru-date-filter"><?php esc_html_e( 'حتى', 'affiliates' ); ?></label>
		...
		<div class="filter-buttons">
			<input class="button apply-button" type="submit" name="apply_filters" value="<?php esc_html_e( 'تطبيق', 'affiliates' ); ?>"/>
			<input class="button clear-button" type="submit" name="clear_filters" value="<?php esc_html_e( 'محو', 'affiliates' ); ?>"/>
		</div>
	</form>
	<?php
		//
...
<div class="earnings-container">
	...
	?>
	<?php else : ?>
		<div class="cell odd full"><?php esc_html_e( 'لا توجد نتائج.', 'affiliates' ); ?></div>
	<?php endif; ?>
...
<?php if ( $section->get_count() > 0 ) : ?>
	<div class="section-navigation">
		<?php if ( $section->get_current_page() > 0 ) : ?>
			<a style="margin: 4px;" class="button" href="<?php echo esc_url( $section->get_url( array( 'earnings-page' => $section->get_current_page() - 1 ) ) ); ?>"><?php echo esc_html_x( 'السابق', 'تم استخدام التصنيف لإظهار الصفحة السابقة لنتائج الارباح', 'affiliates' ); ?></a>
		<?php endif; ?>
		<?php if ( $section->get_current_page() < $section->get_pages() - 1 ) : ?>
			<a style="margin: 4px;" class="button" href="<?php echo esc_url( $section->get_url( array( 'earnings-page' => $section->get_current_page() + 1 ) ) ); ?>"><?php echo esc_html_x( 'التالى', 'تم استخدام التصنيف لإظهار الصفحة التالية لنتائج الارباح', 'affiliates' ); ?></a>
		<?php endif; ?>
	</div>
	<div class="section-navigation-options">
		<form action="<?php echo esc_url( $section->get_url( array( 'per_page' => null ) ) ); ?>" method="post">
			<label class="row-count">
				<?php esc_html_e( 'النتائج لكل صفحة', 'affiliates' ); ?>
				...
				<input class="button" type="submit" value="<?php esc_attr_e( 'تطبيق', 'affiliates' ); ?>"/>
			</label>
		</form>
	</div>
<?php endif; ?>
