...
<?php foreach ( $columns as $key => $column ) : ?>
	<div class="cell <?php echo ( $index === 0 ? 'first' : '' ) . ' ' . ( ( $index + 1 ) % 2 === 0 ? 'even' : 'odd' ); ?> <?php echo esc_attr( $key ); ?>" data-heading="<?php echo esc_attr( $column['title'] ); ?>">
		...
		switch ( $key ) {
			case 'period' :
				echo esc_html( date_i18n( _x( 'F Y', 'فترة الأرباح العام والشهر', 'affiliates' ), strtotime( $entry->year . '-' . $entry->month . '-01' ) ) ); // translators: date format; month and year for earnings display
				break;
			case 'earnings' :
