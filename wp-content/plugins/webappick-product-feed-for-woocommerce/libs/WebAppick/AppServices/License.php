...
class License {
	...
	public function __construct( Client $client ) {
		...
		$this->option_key = 'WebAppick_' . md5( $this->client->slug ) . '_manage_license';
	...
	public function menu_output() {
		...
		$action = ( $license && isset( $license['status'] ) && 'activate' == $license['status'] ) ? 'deactivate' : 'active';
		$this->licenses_style();
		?>
				...
				<div class="webappick-license-details">
					...
					<form method="post" action="<?php $this->formActionUrl(); ?>" novalidate="novalidate" spellcheck="false">
						...
						<div class="license-input-fields">
							...
							<button type="submit" name="submit" class="<?php echo 'deactivate' == $action ? 'deactivate-button' : ''; ?>">
					...
					<?php
					if ( 'deactivate' == $action && isset( $license['remaining'] ) ) {
	...
	private function license_page_form( $form ) {
		switch ( $form['_action'] ) {
			...
			case 'deactivate':
				$this->deactivate_client_license( $form );
				break;
		}
	}
	 ...
	 * @param array $form
	 */
	private function deactivate_client_license( $form ) {
		$license = get_option( $this->option_key, null );
