...
class WPForms_Mailchimp extends WPForms_Provider {
	...
	public function process_entry( $fields, $entry, $form_data, $entry_id = 0 ) {
		...
		foreach ( $form_data['providers'][ $this->slug ] as $connection ) :
			...
			if ( ! $pass ) {
				wpforms_log(
					'MailChimp Subscription stopped by conditional logic',
					$fields,
					array(
			...
			foreach ( $connection['fields'] as $name => $merge_var ) {
				...
				// @todo special formatting for other merge vars (address etc)
				$data[ $name ] = $value;
			}
	...
	public function api_groups( $connection_id = '', $account_id = '', $list_id = '' ) {
		...
		try {
			...
			return $this->error( 'API group error: ' . $e->getMessage() );
		}
	}
	...
	public function api_fields( $connection_id = '', $account_id = '', $list_id = '' ) {
		...
		try {
			...
			if ( ! empty( $provider_fields['data'][0]['merge_vars'] ) ) {
				...
				return $this->error( 'API fields error: No fields' );
			}
		} catch ( Exception $e ) {
			wpforms_log(
				'MailChimp API error',
			...
			return $this->error( 'API fields error: ' . $e->getMessage() );
		}
	}
	...
	public function output_auth() {
		...
		$output .= sprintf( '<h4>%s</h4>', __( 'Add New Account', 'wpforms-mailchimp' ) );

		$output .= sprintf( '<input type="text" data-name="apikey" placeholder="%s %s" class="wpforms-required">', $this->name, __( 'API Key', 'wpforms-mailchimp' ) );

		$output .= sprintf( '<input type="text" data-name="label" placeholder="%s %s" class="wpforms-required">', $this->name, __( 'Account Nickname', 'wpforms-mailchimp' ) );

		$output .= sprintf( '<button data-provider="%s">%s</button>', $this->slug, __( 'Connect', 'wpforms-mailchimp' ) );
	...
	public function output_options( $connection_id = '', $connection = array() ) {
		...
		$output .= sprintf( '<h4>%s</h4>', __( 'Options', 'wpforms-mailchimp' ) );
		...
		$output .= sprintf(
			...
			$connection_id,
			__( 'Send welcome email', 'wpforms-mailchimp' )
		);
		...
		$output .= sprintf(
			...
			$connection_id,
			__( 'Use double opt-in', 'wpforms-mailchimp' )
		);
	...
	public function integrations_tab_new_form() {
		...
		?>
		<input type="text" name="apikey" placeholder="<?php printf( esc_attr__( '%s API Key', 'wpforms-mailchimp' ), $this->name ); ?>">
		<input type="text" name="label" placeholder="<?php printf( esc_attr__( '%s Account Nickname', 'wpforms-mailchimp' ), $this->name ); ?>">
		<?php
	}
}
