...
class Dokan_Pro_Reviews {
    ...
    public function load_review_template( $query_vars ) {
        if ( isset( $query_vars['reviews'] ) ) {
            ...
            if ( ! current_user_can( 'dokan_view_review_menu' ) ) {
                dokan_get_template_part('global/dokan-error', '', array( 'deleted' => false, 'message' => __( 'ليس لديك إذن لعرض صفحة التقييمات', 'dokan' ) ) );
                return;
            } else {
    ...
    function ajax_comment_status() {
        ...
        if ( ! current_user_can( 'dokan_manage_reviews' ) ) {
            wp_send_json_error( __( 'ليس لديك إذن لإدارة هذا الاستعراض', 'dokan' ) );
            return;
        }
    ...
    function ajax_update_comment() {
        if ( ! $this->quick_edit ) {
            wp_send_json_error( __( 'لا يمكنك تعديل التعليقات!', 'dokan' ) );
        }
    ...
    function render_store_tab_comment_list( $comments, $store_id ) {
        ...
        ob_start();
        if ( count( $comments ) == 0 ) {
            echo '<span colspan="5">' . __( 'لم يتم العثور على تعليقات', 'dokan' ) . '</span>';
        } else {
